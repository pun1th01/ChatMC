...
ChatMC provides a secondary, opt-in communication channel that operates independently of the vanilla chat system. It uses custom network packets for messaging, meaning ChatMC messages are entirely separate from vanilla chat. Both systems can coexist, allowing players to switch between them as needed.

Server operators who install ChatMC have full control over this channel, including local chat logging and moderation tools.
...
3. Place the `chatmc-x.x.x.jar` file in your `mods/` directory.
...
- **Toggle Keybind** — Press **Right Shift** to switch between ChatMC mode and vanilla chat.
- **HUD Indicator** — A `[ChatMC Active]` badge appears at the top center of the screen when active.
- **Chat Bar Indicator** — A teal `[ChatMC]` label appears in the chat input field when typing in ChatMC mode.
- **Chat History** — On join, the last 20 messages from the current day's log are displayed in italic grey with a `--- ChatMC History ---` header.
- **Background Opacity** — Configurable transparency for the ChatMC overlay background.
- **Server-Side Mutes** — Operators can silently mute players, dropping their ChatMC messages server-side.
- **Flat-File Logging** — Messages, joins, and leaves are logged to daily local files.
- **Graceful Fallback** — Displays a notice if the server does not support ChatMC or if it is disabled.
...
| Command | Description |
|---|---|
| `/chatmc mute <player>` | Silently drops the specified player's ChatMC messages server-side. |
| `/chatmc unmute <player>` | Removes the mute from the specified player. |
| `/chatmc toggle` | Toggles ChatMC on or off for the server and updates the config file. |
...
|---|---|---|---|
| `enabled` | boolean | `true` | Whether ChatMC is active on the server. |
...
Managed via `/chatmc mute` and `/chatmc unmute`. Stores a JSON array of UUID strings. Manual edits require a server restart.

## Logging and Data

Chat logs are saved locally on the server in `config/chatmc/logs/` as daily flat files (`YYYY-MM-DD.log`). Old logs are automatically deleted on server start based on the `log-retention-days` setting.
...
- Unmodded clients cannot send or receive ChatMC messages.
